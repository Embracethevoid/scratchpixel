...
class Sphere
{
  public:
    ...
    /**
     * This function requires that the rayDireaction is normalized 
     * 
     * 
     */
    bool intersect(const Vec3d &rayOrigin, const Vec3d &rayDirection, double &t0, double &t1) const
    {
        // l is the vector from rayOrigin to the sphere center
        Vec3d l = center - rayOrigin;

        double tca = l.dot(rayDirection);
        // the angel between the light and the shpere > 90
        if (tca < 0)
            return false;
    ...
    const int &depth)
{
    ...
    if (rayDirection.dot(normalAtIntersection) > 0)
        normalAtIntersection = -normalAtIntersection, inside = true;
    ...
    if ((sphere->transparency > 0 || sphere->reflection > 0) && depth < MAX_RAY_DEPTH)
    {
        ...
        {
            double ior = 1.5, eta = inside ? ior : 1 / ior;
            ...
            refraction = trace(pointOfIntersection - normalAtIntersection * bias,
                               refractionDirection, spheres, depth + 1);
        }
...
void render(const std::vector<Sphere> &spheres)
{
    ...
    std::ofstream ofd("./self.txt", std::ios::out);
    std::ofstream ofs("./self.ppm", std::ios::out | std::ios::binary);
    ofs << "P6\n"
        << width << " " << height << "\n255\n";
    for (unsigned i = 0; i < width * height; ++i)
    {
        ofs << (unsigned char)(std::min(double(1), image[i].x) * 255) << (unsigned char)(std::min(double(1), image[i].y) * 255) << (unsigned char)(std::min(double(1), image[i].z) * 255);
    }
    ofs.close();
    ofd.close();
    delete[] image;
...
int main(int argc, char **argv)
{
    ...
    spheres.push_back(Sphere(Vec3d(0.0, -10004, -20), 10000, Vec3d(0.20, 0.20, 0.20), 0, 0.0));
    spheres.push_back(Sphere(Vec3d(0.0, 0, -20), 4, Vec3d(1.00, 0.32, 0.36), 1, 0.5));
    spheres.push_back(Sphere(Vec3d(5.0, 0, -15), 2, Vec3d(0.90, 0.0, 0.0), 1, 0));
    spheres.push_back(Sphere(Vec3d(0.0, 0, -25), 3, Vec3d(0.0, 0.9, 0.9), 1, 0));
    spheres.push_back(Sphere(Vec3d(-5.5, 0, -15), 3, Vec3d(0.90, 0.90, 0.90), 1, 0.0));
    // light
    spheres.push_back(Sphere(Vec3d(0.0, 20, -30), 3, Vec3d(0.00, 0.00, 0.00), 0, 0.0, Vec3d(3)));
    render(spheres);
